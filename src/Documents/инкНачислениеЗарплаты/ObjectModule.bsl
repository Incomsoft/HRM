...
Процедура ДобавитьДвижение_ОсновноеНачисление(ВедомостьСтрока,инкНачисления,Начисление,Сумма)
	...
	Если Год(ЭтотОбъект.МесяцНачисления) < 2025 Тогда
	...
	Иначе
		Движение.РайонныйКоэффициент = 0;
		Движение.СевернаяНадбавка    = 0;
	КонецЕсли;
	...
	ОтразитьДвижениеРКиСНВРегистре(ПланыВидовРасчета.инкНачисления.РайонныйКоэффициент,ВедомостьСтрока.РайонныйКоэффициентОклад,ВедомостьСтрока);
	ОтразитьДвижениеРКиСНВРегистре(ПланыВидовРасчета.инкНачисления.СевернаяНадбавка,ВедомостьСтрока.СевернаяНадбавкаОклад,ВедомостьСтрока);
...
Процедура ДобавитьДвижение_ДополнительноеНачисление(инкНачисления,ПрочиеНачисленияСтрока)
	...
	ОтразитьДвижениеРКиСНВРегистре(ПланыВидовРасчета.инкНачисления.РайонныйКоэффициент,ПрочиеНачисленияСтрока.РайонныйКоэффициент,ПрочиеНачисленияСтрока);
	ОтразитьДвижениеРКиСНВРегистре(ПланыВидовРасчета.инкНачисления.СевернаяНадбавка,ПрочиеНачисленияСтрока.СевернаяНадбавка,ПрочиеНачисленияСтрока);
...
Процедура ОтразитьДвижениеРКиСНВРегистре(ВидНачисления,НачислениеСумма,ВедомостьСтрока)

	Если Год(ЭтотОбъект.МесяцНачисления) < 2025 Тогда
		Возврат;
	КонецЕсли;
	
	Если НачислениеСумма = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Движение = ЭтотОбъект.Движения.инкНачисления.Добавить(); 
	
	Движение.ИсточникФинансирования = ЭтотОбъект.ИсточникФинансирования;
	Движение.Начисление			= ВидНачисления;
	Движение.Период 		   	= НачалоМесяца(ЭтотОбъект.МесяцНачисления);
	Движение.Сотрудник			= ВедомостьСтрока.Сотрудник;
	Движение.ДокументОснование 	= ЭтотОбъект.Ссылка;
	Движение.ДоходНДФЛ 			= Справочники.инкДоходНДФЛ.Код2006;
	Движение.Организация 		= ЭтотОбъект.Организация;
	Движение.Подразделение 		= ВедомостьСтрока.Подразделение;
	Движение.ВычетНДФЛ 			= Справочники.инкВычетыНДФЛ.ПустаяСсылка();
	Движение.Налог 				= Истина;
	...
	Движение.РайонныйКоэффициент = 0;
	Движение.СевернаяНадбавка    = 0;
	Если ВидНачисления = ПланыВидовРасчета.инкНачисления.РайонныйКоэффициент Тогда
		Движение.РайонныйКоэффициент = НачислениеСумма;
	Иначе
		Движение.СевернаяНадбавка    = НачислениеСумма;	
	КонецЕсли;
	//
	
	Движение.Отпуск				= ВидНачисления.Отпуск;
	Движение.ВходитВФОТ			= ВидНачисления.ВходитВФОТ;
	Движение.БольничныйЛист		= ВидНачисления.БольничныйЛист; 
	Движение.МатериальнаяПомощь = ВидНачисления.МатериальнаяПомощь;
