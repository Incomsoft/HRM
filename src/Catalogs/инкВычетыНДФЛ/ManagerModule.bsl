...
Процедура ОписатьКодВычетаНДФЛ(ИмяПредопределенныхДанных, Код, Наименование, ВычетКДоходу = Истина, ГруппаВычета = Неопределено, Код2010 = "", Код2011 = "", Код2015 = "", Код2016 = "", НеПредоставляетсяНерезидентам = Ложь, ПорядковыйНомерВРасчетеНалога = 0, СуммаВычета = 0) 

	// АПК:1327-выкл используется при начальном заполнении ИБ.
	
	СсылкаПредопределенного = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.инкВычетыНДФЛ." + ИмяПредопределенныхДанных);
	Если ЗначениеЗаполнено(СсылкаПредопределенного) Тогда
		Элемент = СсылкаПредопределенного.ПолучитьОбъект();
	Иначе
		Элемент = Справочники.инкВычетыНДФЛ.СоздатьЭлемент();
	...
	Если СуммаВычета <> 0 Тогда
		Если Элемент.СуммаВычета <> СуммаВычета Тогда
			Элемент.СуммаВычета = СуммаВычета;
		КонецЕсли;		
	КонецЕсли;
	Если ЗначениеЗаполнено(ГруппаВычета) Тогда
		Элемент.ГруппаВычетаНДФЛ = ГруппаВычета; 	
	КонецЕсли;
...
//
Процедура ЗаполнитьВычетыНДФЛ() Экспорт
	
	ГруппыВычетов = Перечисления.инкГруппыВычетовПоНДФЛ;
...
Функция ПолучитьСкрытыеВычетыМассив() Экспорт
	
	СкрытыеВычетыМассив = Новый Массив;
	СкрытыеВычетыМассив.Добавить(Справочники.инкВычетыНДФЛ.Код115);
	СкрытыеВычетыМассив.Добавить(Справочники.инкВычетыНДФЛ.Код116);
	СкрытыеВычетыМассив.Добавить(Справочники.инкВычетыНДФЛ.Код131);
	СкрытыеВычетыМассив.Добавить(Справочники.инкВычетыНДФЛ.Код117о);
	
	Возврат СкрытыеВычетыМассив;
