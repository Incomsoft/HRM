...
Процедура ЗаполнитьФормуДаннымиПоУмолчанию()
	...
	//Объект.ПутьКФайлу =	"d:\1С\!Инкомсофт\!Текущие задачи\issue2 - модифицировать загрузку из старой программы в новую\1C-2022_04_14-ORG-009-001-035150.XML";
...
Процедура ЗагрузитьДокументы(Команда)
	...
	КлючеваяОперация = "Обработка.инкЗагрузкаВнешнихНачисленийИнкомсофт10_5.ЗагрузкаДанных";
...
Процедура ЗагрузитьЛогиНаСервере(АдресВоВремХранилище)
	...
	Для каждого ДокументСтрока из ИсходныеДанные.ДокументыНачисленияЗарплаты Цикл
		...
		СтрокаОтчет.СформированныйДокумент = ДокументСтрока;
...
Процедура ЗагрузитьИсходныеДанныеДляЗаписиXML(ИсходныеДанные)
	...
	ИсходныеДанные.Вставить("СотрудникиТаблица",Справочники.инкСотрудники.ПолучитьСписокСотрудниковСКадровымиДанными()); 
	//
...
Функция ПолучитьДанныеПлановВидовРасчета(ИмяПлана)
	...
	Для Каждого Элемент Из ТаблицаЗначений Цикл
		Элемент.Наименование = НРег(Элемент.Наименование);		
	КонецЦикла;
	...
	Для каждого элБазаДанных из ДокументДОМ.ЭлементДокумента.ДочерниеУзлы Цикл
		...
		ИначеЕсли элБазаДанных.ИмяЭлемента = "ПрочиеНачисления" Тогда
			...
			Для каждого элНачисление Из элБазаДанных.ДочерниеУзлы Цикл
				...
				КонецЦикла;
				НачислениеXML.Вставить("ЭтоУдержание",УдержаниеВНачислении(НачислениеXML));
		...
		ИначеЕсли элБазаДанных.ИмяЭлемента = "ДанныеСотрудников" Тогда
			...
			Для каждого элСотрудник Из элБазаДанных.ДочерниеУзлы Цикл
				...
				Для каждого элДанныеСотрудника Из элСотрудник.ДочерниеУзлы Цикл
					...
					// Банк						
					ИначеЕсли элДанныеСотрудника.ИмяЭлемента = "Банк" Тогда  	
						
						БанкСтруктура = Новый Структура;
						Для каждого элДанные Из элДанныеСотрудника.ДочерниеУзлы Цикл
							БанкСтруктура.Вставить(элДанные.ИмяЭлемента,элДанные.ТекстовоеСодержимое);	
						КонецЦикла;
						СотрудникXML.Вставить("Банк",БанкСтруктура);						
				...
				Для каждого элНачисленияИУдержания Из элВедомость.ДочерниеУзлы Цикл
					...
					Если элНачисленияИУдержания.ИмяЭлемента = "Начисления"  Тогда
						Для каждого элНачислениеВедомостиXML Из элНачисленияИУдержания.ДочерниеУзлы Цикл
							...
							КонецЦикла;    
							стрДанныеНачисления.Вставить("ЭтоУдержание",УдержаниеВНачислении(стрДанныеНачисления));
		...
		Для каждого элУдержаниеВедомости Из мУдержанияВедомости Цикл
			...
			Для каждого элСотрудник Из МассивСотрудниковXML Цикл
				Если элУдержаниеВедомости.СотрудникКод = элСотрудник.СотрудникКод Тогда
					...
					элУдержаниеВедомости.Вставить("ПодразделениеСсылка",элСотрудник.Подразделение);		
					Прервать;				
				КонецЕсли;				
			КонецЦикла;      
...
Процедура СформироватьДокумент_ПриказОПриемеНаРаботуПоАнкетнымДанным(ИсходныеДанные,СотрудникXML)
	...
	Если Не ПроверкаСотрудникаНаНаличиеДок(СотрудникXML, ОрганизацияXML, ПриемНаРаботуТаблица) Тогда
		...
		ПриемНаРабДок.ДатаПриказа	  = ДатаПриемаНаРаботу;
		//
		...
		//
		ПриемНаРабДок.Комментарий     = "Создан автоматически обработкой:""Загрузка внешних начислений (Инкомсофт: зарплата и кадры 10.5)"" (по анкетным данным сотрудника).";
		...
		ПриемНаРабДок.СевернаяНадбавка = Число(СотрудникXML.КН);
		ПриемНаРабДок.РайонныйКоэффициент = Число(СотрудникXML.РК);
		//
	...
	Для каждого элПриказОПриемеНаРаботу из мПриказыОПриемеНаРаботу Цикл
		...
		Если элПриказОПриемеНаРаботу.НомерСотрудника = СотрудникXML.СотрудникКод Тогда
			...
			Если Не ПроверкаСотрудникаНаНаличиеДок(СотрудникXML, ОрганизацияXML, ПриемНаРаботуТаблица) Тогда
				...
				ПриемНаРабДок.ДатаПриказа = элДата;
				//
				ПриемНаРабДок.Комментарий     = "Создан автоматически обработкой:""Загрузка внешних начислений (Инкомсофт: зарплата и кадры 10.5)"" (по анкетным данным сотрудника).";
				ПриемНаРабДок.ИдентификаторЗагрузки = ИсходныеДанные.ИдентификаторЗагрузки;
	...
	Для каждого элПриказОбУвольнении из мПриказыОбУвольнении Цикл
		Если элПриказОбУвольнении.НомерСотрудника = СотрудникXML.СотрудникКод Тогда
			Если Не ПроверкаСотрудникаНаНаличиеДок(СотрудникXML, ОрганизацияXML, УвольнениеТаблица) Тогда
				...
				УвольнениеДок.Подразделение   = СотрудникXML.Подразделение;
				УвольнениеДок.Должность   	  = СотрудникXML.Должность;
				...
				УвольнениеДок.ДатаПриказа  = элДата;
				//     				
				УвольнениеДок.Сотрудник       = СотрудникXML.СотрудникСсылка;
				УвольнениеДок.Основание		  = элПриказОбУвольнении.ОснованиеПриказа; 
	...
	Для каждого элПриказыОКадровыхИзменениях из мПриказыОКадровыхИзменениях Цикл
		Если элПриказыОКадровыхИзменениях.НомерСотрудника = СотрудникXML.СотрудникКод Тогда
			Если Не ПроверкаСотрудникаНаНаличиеДок(СотрудникXML, ОрганизацияXML, КадровыйПереводТаблица) Тогда
				...
				КадровыйПереводДок.Основание = элПриказыОКадровыхИзменениях.Основание;	
...
Функция ЗаписьДанныхСотрудника(ИсходныеДанные, СотрудникXML);
	...
	Если ЗначениеЗаполнено(СотрСсылка) Тогда
		
		СотрудникОбъект = СотрСсылка.ПолучитьОбъект();	
		Если ЗначениеЗаполнено(СотрудникXML.Банк.БанковскийСчет) Тогда
			
			СотрудникОбъект.ЛицевойСчет = СотрудникXML.Банк.БанковскийСчет;      
			Если ЗначениеЗаполнено(СотрудникXML.Банк.НаименованиеБанка) Тогда
				СотрудникОбъект.ЗарплатныйПроект = Справочники.инкЗарплатныеПроекты.НайтиПоНаименованию(СотрудникXML.Банк.НаименованиеБанка);
			КонецЕсли;
...
Процедура ЗаписатьНачисления(ИсходныеДанные)
	...
	// Загрузка обычных начислений:
	Для каждого элНачисление Из МассивНачисленийXML Цикл
		
		Если (элНачисление.ВПроцентах = "+") Тогда
			Продолжить;	
		КонецЕсли;
	...
	// Загрузка начислений расчет которых зависит от других начислений:
	Для каждого элНачисление Из МассивНачисленийXML Цикл
		
		Если НЕ (элНачисление.ВПроцентах = "+") Тогда
			Продолжить;	
		КонецЕсли; 
...
Процедура ОбработатьУдержание(элУдержание,МассивУдержанийXML,УдержанияТаблица,ИсходныеДанные)
	...
	КодУдержания = ВРег(элУдержание.КодРасчета);
	
	// У нас некоторые начисления могут быть удержаниями. Такие удержания начинаются с 20000:
	Если Лев(КодУдержания, 1) = "2" Тогда
		КодУдержания = "0"+Сред(КодУдержания,2,СтрДлина(КодУдержания) - 1);
	КонецЕсли;
	
	// Прямой поиск:
    Если 	  КодУдержания = "00080"  Тогда
		УдержаниеСсылка = ПланыВидовРасчета.инкУдержания.Аванс;	    
    ИначеЕсли КодУдержания = "00081" Тогда 
    	УдержаниеСсылка = ПланыВидовРасчета.инкУдержания.МежрасчетныеВыплаты;   
    ИначеЕсли КодУдержания = "00400" Тогда 
    	УдержаниеСсылка = ПланыВидовРасчета.инкУдержания.ВыплатаОтпускныхИБЛВТекущемМесяце;  
    ИначеЕсли КодУдержания = "00097" Тогда 
    	УдержаниеСсылка = ПланыВидовРасчета.инкУдержания.Алименты;  
    ИначеЕсли КодУдержания = "00500" Тогда 
    	УдержаниеСсылка = ПланыВидовРасчета.инкУдержания.ЗарплатаПредыдущегоМесяца;  
    ИначеЕсли КодУдержания = "00501" Тогда 
    	УдержаниеСсылка = ПланыВидовРасчета.инкУдержания.ЗарплатаТекущегоМесяца;  
    КонецЕсли;  

	// Поиск по коду:
    Если НЕ ЗначениеЗаполнено(УдержаниеСсылка) Тогда
	    
	    ПоискСтруктура = Новый Структура("КодВСтаройПрограмме",Число(элУдержание.КодРасчета));
	    УдержаниеМассив = УдержанияТаблица.НайтиСтроки(ПоискСтруктура);
	    
	    Для каждого УдержаниеЭлемент из УдержаниеМассив Цикл
	...
	Если НЕ УдержаниеСсылка.Предопределенный Тогда
...
Процедура ОбработатьНачисление(элНачисление,МассивНачисленийXML,НачисленияТаблица,ИсходныеДанные)
    ...
    // Прямой поиск:
    Если ВРег(элНачисление.КодРасчета) = "ОКЛ" Тогда
		НачислениеСсылка = ПланыВидовРасчета.инкНачисления.Оклад;	    
    ИначеЕсли ВРег(элНачисление.КодРасчета) = "Р/К" Тогда 
    	НачислениеСсылка = ПланыВидовРасчета.инкНачисления.РайонныйКоэффициент;   
    ИначеЕсли ВРег(элНачисление.КодРасчета) = "С/Н" Тогда 
    	НачислениеСсылка = ПланыВидовРасчета.инкНачисления.СевернаяНадбавка;
    ИначеЕсли ВРег(элНачисление.КодРасчета) = "САЛЬДО_ВХОДЯЩЕЕ" Тогда 
    	НачислениеСсылка = ПланыВидовРасчета.инкНачисления.Сальдо;
    КонецЕсли;  

	// Поиск по коду:
    Если НЕ ЗначениеЗаполнено(НачислениеСсылка) Тогда
	    
	    ПоискСтруктура = Новый Структура("КодВСтаройПрограмме",Число(элНачисление.КодРасчета));
    ...
    Если НЕ ЗначениеЗаполнено(НачислениеСсылка) Тогда
	    ...
	    ПоискСтруктура = Новый Структура("Наименование",НРег(элНачисление.Наименование));
	    НачисленияМассив = НачисленияТаблица.НайтиСтроки(ПоискСтруктура);
	...
	Если НЕ НачислениеСсылка.Предопределенный Тогда
		...
		//
		КодВСтаройПрограммеЧисло = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(НачислениеСсылка.КодВСтаройПрограмме);
		Если (КодВСтаройПрограммеЧисло >= 100) И (КодВСтаройПрограммеЧисло <= 130) Тогда
			НачислениеОбъект.ВходитВФОТ = Ложь;
		Иначе
	...
	// Обрабатываем базовые начисления:
	Если НачислениеСсылка.ВПроцентах Тогда
		...
		//
		КодыНачисленийМассив = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(элНачисление.CREAPROC, "+");
		//
		
		Для каждого КодыНачисленийЭлемент Из КодыНачисленийМассив Цикл
			
			БазовоеНачислениеСсылка = ПланыВидовРасчета.инкНачисления.ПустаяСсылка();
			
			ПоискСтруктура = Новый Структура("КодВСтаройПрограмме",Число(КодыНачисленийЭлемент));
		    НачисленияМассив = НачисленияТаблица.НайтиСтроки(ПоискСтруктура);
		    
		    Для каждого НачислениеЭлемент из НачисленияМассив Цикл
		        БазовоеНачислениеСсылка = НачислениеЭлемент.Ссылка;
		    КонецЦикла;

			Если ЗначениеЗаполнено(БазовоеНачислениеСсылка) Тогда

				БазовыйВидРасчета = НачислениеОбъект.БазовыеВидыРасчета.Добавить();
				БазовыйВидРасчета.ВидРасчета = БазовоеНачислениеСсылка;
...
Функция ПолучитьЗначениеПоВзносам(ЗначениеПараметра)
	...
	// Пояснение. Например начисление входит во взносы, но исключается по ст.9ФЗ 212. Тогда галочка должна стоять
	// во взносах и в ст.9 ФЗ 212.
...
Функция ЗаписьДанныхПодразделения(ИсходныеДанные,элПодразделение,ссОрганизация)
	...
	Подразделения = ПодразделенияТаблица.НайтиСтроки(ПоискСтруктура);
	
	Для каждого ПодразделениеЭлемент из Подразделения Цикл
...
Функция ПолучитьСсылкуОрганизации(ИсходныеДанные,ОрганизацияXML,ОрганизацииТаблица)
	...
	ОргЭлемент.СистемаНалогооблажения = ПолучитьВидОсновнойДеятельности(ОрганизацияXML.СН);
	ОргЭлемент.Записать();
...
Функция ПолучитьВидОсновнойДеятельности(СН)
	...
	ВидОсновнойДеятельности = Перечисления.инкСистемаНалогооблажения.ПустаяСсылка();
	
	Если СН = "ОСНО" Тогда
		ВидОсновнойДеятельности = Перечисления.инкСистемаНалогооблажения.ОСНО;
	ИначеЕсли СН = "УСНО" Тогда
		ВидОсновнойДеятельности = Перечисления.инкСистемаНалогооблажения.УСНО;
	ИначеЕсли СН = "ЕНВД" Тогда
		ВидОсновнойДеятельности = Перечисления.инкСистемаНалогооблажения.ЕНВД;
	КонецЕсли;		
...
Процедура ЗаписатьНачислениеЗарплатыНаСервере(ИсходныеДанные,ВедомостьСтрока,Отказ)
	...
	дНачислениеЗП.ВидРасчетаДниЧасы = Перечисления.инкВидРасчетаДниЧасы.ВДнях;
	...
	Для каждого НачислениеСтрока из мНачисленийВедомости Цикл
		Сотрудники.Добавить(НачислениеСтрока.СотрудникСсылка);
	КонецЦикла;
	Для каждого УдержаниеСтрока из мУдержанийВедомости Цикл
		Сотрудники.Добавить(УдержаниеСтрока.СотрудникСсылка);
	КонецЦикла;
	Сотрудники = инкОбщийСервер.УдалитьДублиИзМассива(Сотрудники);
	...
	Для каждого СотрудникЭлемент из Сотрудники Цикл
		...
		Для каждого элНачисление Из мНачисленийВедомости Цикл
			...
			Если элНачисление.СотрудникСсылка = СотрудникЭлемент Тогда
				...
				Если элНачисление.НачислениеКод = "ОКЛ" Тогда
					...
					// В файле переносятся не часы, а норма времени. Мы ее запишем, как норму дни:
					//ВедомостьСтрока.Часы        = элНачисление.Часы;	                  
					//ВедомостьСтрока.ЧасыФакт   	= элНачисление.ЧасыФакт;	          
					ВедомостьСтрока.НормаДни        = элНачисление.Часы;	
					ВедомостьСтрока.НормаДниФакт   	= элНачисление.ЧасыФакт;	
		...
		КонецЦикла;
		ВедомостьСтрока.Оклад 		= ОкладСумма;		
