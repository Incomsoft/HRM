...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если инкОбщийКлиентСервер.ЕстьСвойство(Параметры,"Организация") Тогда
		...
		Элементы.мОрганизация.Видимость = Истина;  
		Если ЗначениеЗаполнено(Параметры.Организация) Тогда
			мОрганизация = Параметры.Организация;
		Иначе
...
Процедура ВыполнитьВыборПериода()
	...
	РезультатВыбора.Вставить("Организация",мОрганизация);
	РезультатВыбора.Вставить("Дата1",РезультатВыбора.НачалоПериода);
	РезультатВыбора.Вставить("Дата2",РезультатВыбора.КонецПериода);
	
	ПериодВЗапросе.ДатаНачала = РезультатВыбора.НачалоПериода;
	ПериодВЗапросе.ДатаОкончания = РезультатВыбора.КонецПериода;
	РезультатВыбора.Вставить("ПериодВЗапросе",ПериодВЗапросе);
