...
Процедура ОписатьКодДоходаНДФЛ(СсылкаИлиИмяПредопределенныхДанных = Неопределено, СтавкаНалогообложения = Неопределено, Код = "", Наименование = "", ВычетРассчитываетсяАвтоматически = Ложь, ИмеетЕдинственныйВычет = Ложь, ВычетПоУмолчанию = Неопределено, НеОблагаетсяУНалоговогоАгента = Ложь, СоответствуетОплатеТруда = Ложь)
	...
	Если ТипЗнч(СсылкаИлиИмяПредопределенныхДанных) = Тип("Строка") И Не ПустаяСтрока(СсылкаИлиИмяПредопределенныхДанных) Тогда
		...
		СсылкаПредопределенного = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.инкДоходНДФЛ." + СсылкаИлиИмяПредопределенныхДанных);
		Если ЗначениеЗаполнено(СсылкаПредопределенного) Тогда
			Элемент = СсылкаПредопределенного.ПолучитьОбъект();
		Иначе
			Элемент = Справочники.инкДоходНДФЛ.СоздатьЭлемент();
	...
	Иначе
		...
		Иначе
			СсылкаНаЭлемент = Справочники.инкДоходНДФЛ.НайтиПоКоду(Код);
		КонецЕсли; 
	...
	Элемент.СтавкаНалогообложенияРезидента		= ?(Не ЗначениеЗаполнено(СтавкаНалогообложения), Перечисления.инкНДФЛСтавкиНалогообложенияРезидента.Ставка13, СтавкаНалогообложения);
	Элемент.ВычетНДФЛ							= ВычетПоУмолчанию;
...
Процедура Обновление_11_0_2_72() Экспорт
	
	Ставка13 = Перечисления.инкНДФЛСтавкиНалогообложенияРезидента.Ставка13;
	
	ОписатьКодДоходаНДФЛ("Код2006", Ставка13, "2006", "Оплата труда, получаемая лицами, работающими в районах Крайнего Севера с неблагоприятными (особыми) климатическими или экологическими условиями", Ложь, Ложь);
...
Процедура ЗаполнитьКодыДоходов2014()
	...
	Ставка13 = Перечисления.инкНДФЛСтавкиНалогообложенияРезидента.Ставка13;
	ВычетыНДФЛ = Справочники.инкВычетыНДФЛ;	
...
Процедура ЗаполнитьКодыДоходов2018()
	...
	ОписатьКодДоходаНДФЛ( , Перечисления.инкНДФЛСтавкиНалогообложенияРезидента.Ставка35, "3023", "Сумма дохода, получаемого по обращающимся облигациям, номинированным в рублях и эмитированным после 1 января 2017 года");
...
Процедура ЗаполнитьКодыДоходовНДФЛ()
	...
	Ставка09 = Перечисления.инкНДФЛСтавкиНалогообложенияРезидента.Ставка09;
	Ставка13 = Перечисления.инкНДФЛСтавкиНалогообложенияРезидента.Ставка13;
	Ставка35 = Перечисления.инкНДФЛСтавкиНалогообложенияРезидента.Ставка35;
	ВычетыНДФЛ = Справочники.инкВычетыНДФЛ;
...
//
Процедура СоздатьКодыДоходовНДФЛ() 
...
Процедура ОбновлениеДоходовВычетовНДФЛНеПредопределенныхЯнварь2012()
	...
	|ИЗ
	|	Справочник.инкДоходНДФЛ КАК ДоходыНДФЛ
	|ГДЕ
	|	ДоходыНДФЛ.Код = ""1541""
...
Процедура ПроставитьКодамНатуральныхДоходовПризнакПредопределенного()
	...
	|ИЗ
	|	Справочник.инкДоходНДФЛ КАК ВидыДоходовНДФЛ
	|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		ЭлементДляУдаления = Справочники.инкДоходНДФЛ[ИмяПредопределенныхДанных].ПолучитьОбъект();		
...
// Обработчик обновления УТ 11.0.0.1
//
// Первоночальное заполнение элементов справочника 
// 
Процедура ЗаполнитьДоходыНДФЛ() Экспорт
