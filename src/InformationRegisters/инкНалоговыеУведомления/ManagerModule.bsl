...
Функция ПолучитьНалоговыеУведомленияТаблица(ДанныеЗапроса) Экспорт
	...
	Запрос.Текст = 
		...
		|	СУММА(инкНалоговыеУведомления.АвансНДФЛСумма) КАК АвансНДФЛСумма,
		|	СУММА(инкНалоговыеУведомления.ВзносыСумма) КАК ВзносыСумма,
		|	СУММА(инкНалоговыеУведомления.НДФЛСумма) КАК НДФЛСумма,
		|	СУММА(ЕСТЬNULL(инкНалоговыеУведомления.АвансНДФЛСумма, 0) + ЕСТЬNULL(инкНалоговыеУведомления.НДФЛСумма, 0)) КАК НалогСумма
		|ИЗ
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	инкНалоговыеУведомления.Период,
		|	инкНалоговыеУведомления.Организация";
