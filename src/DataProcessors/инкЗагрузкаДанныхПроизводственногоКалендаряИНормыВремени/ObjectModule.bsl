...
// Процедура - Очистить производсвенный календарь
//
// Параметры:
//  Год	 - Число - Год;
//
...
Процедура ОчиститьПроизводсвенныйКалендарь(Год = 0) Экспорт
	...
	ОбщегоНазначения.СообщитьПользователю("Данные по производсвенному календарю очищены.");
	
КонецПроцедуры                                

// Процедура - Очистить норму времени
//
// Параметры:
//  Год	 - Числ - Год;
//
...
Процедура ОчиститьНормуВремени(Год = 0) Экспорт
	...
	ОбщегоНазначения.СообщитьПользователю("Данные по норме времени очищены.");
...
&НаСервере
Процедура ЗагрузитьДанныеНаСервере(Год,ПроверитьНаличиеДанных = Ложь) Экспорт
	
	ЗагрузитьДанныеПроизводственногоКалендаря(Год,ПроверитьНаличиеДанных);
	инкУчетВремениСервер.ЗагрузитьНормуВремениИзГодовогоФондаРабочегоВремениНаСервере(Год,ПроверитьНаличиеДанных);
  	
КонецПроцедуры

&НаСервере
Процедура ЗагрузитьДанныеПроизводственногоКалендаря(Год,ПроверитьНаличиеДанных)
	...
	НаборЗаписейПК.Прочитать();
	
	Если ПроверитьНаличиеДанных И НаборЗаписейПК.Количество() > 0 Тогда
		Возврат;
	КонецЕсли;
	
	НаборЗаписейПК.Очистить();

	ПроизводственныйКалендарьРФ = инкПроизводственныйКалендарьСервер.ПолучитьПроизводственныйКалендарьРФ(); 
	...
	ОбщегоНазначения.СообщитьПользователю("Данные за "+Год+" год загружены в производственный календарь.");
